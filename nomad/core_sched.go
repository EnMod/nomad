...
func (c *CoreScheduler) gcPastVolumeClaims(job *structs.Job) error {
	...
	for _, taskGroup := range job.TaskGroups {
		for _, tgVolume := range taskGroup.Volumes {
			...
			if err != nil || vol == nil {
				// TODO(tgross): maybe just continue if the volume isn't here?
				// it could have been deregistered out from under us.
				return err
			}
			for alloc, _ := range vol.PastAllocs {
				err := c.srv.controllerUnpublishVolume(alloc, volID)
				if err != nil {
					return err
				}

				// the Allocation pointer in PastAllocs is nil and the alloc
				// might have been GC'd concurrently, so we make a new empty
				// Allocation just so the volume claim handler can deference
				// the ID.
				// TODO(tgross): should we change the claim so that we only *need*
				// the allocation ID?
				req := &structs.CSIVolumeClaimRequest{
					VolumeID:   volID,
					Allocation: &structs.Allocation{ID: alloc},
